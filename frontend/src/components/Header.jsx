...
const CustomHeader = ({ currentUser, onLogout }) => {
  ...
  useEffect(() => {
    ...
    // 如果用户信息中没有avatar，尝试从后端获取完整信息
    const fetchFullUserInfo = async () => {
      const user = currentUser || (localStorage.getItem('userInfo') ? JSON.parse(localStorage.getItem('userInfo')) : null)
      if (user && !user.avatar) {
        try {
  ...
  useEffect(() => {
    ...
    } else if (pathname === '/product-management') {
      setSelectedKeys(['product-management'])
      ...
      <div className="header-content">
        <div className="header-left">
          ...
          <Link to="/">校园二手商品交易系统</Link>
            <img 
              src="/img/65313baf318bc7b23c79fee2505cc41c.png"
              alt="武汉理工大学" 
          ...
          >
            ...
            {userInfo && userInfo.user_type === 'admin' && (
              ...
              <Menu.Item key="product-management" icon={<BookOutlined />}>
                <Link to="/product-management">商品管理</Link>
              </Menu.Item>
            ...
            <>
              ...
              <Dropdown overlay={userMenu} trigger={['click']}>
                <div className="user-info">
                  <Avatar size="large" src={userInfo?.avatar || null}>
